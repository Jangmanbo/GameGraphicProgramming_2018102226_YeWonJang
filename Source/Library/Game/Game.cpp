...
namespace library
{
    ...
    HRESULT InitWindow(_In_ HINSTANCE hInstance, _In_ INT nCmdShow)
    {
        ...
        g_hWnd = CreateWindow(wcex.lpszClassName, L"Game Graphics Programming Lab 01: Direct3D 11 Basics",
    ...
    HRESULT InitDevice()
    {
        ...
        ComPtr<IDXGIFactory1> dxgiFactory = nullptr;
        {
            ...
            hr = g_pd3dDevice.CopyTo(IID_PPV_ARGS(&dxgiDevice));
            if (SUCCEEDED(hr))
            {
        ...
        hr = dxgiFactory.CopyTo(IID_PPV_ARGS(&dxgiFactory2));
        if (dxgiFactory2)
        {
            hr = g_pd3dDevice.CopyTo(IID_PPV_ARGS(&g_pd3dDevice1));
            if (SUCCEEDED(hr))
            {
                (void)g_pImmediateContext.CopyTo(IID_PPV_ARGS(&g_pImmediateContext1));
            }
            ...
            if (SUCCEEDED(hr))
            {
                hr = g_pSwapChain1.CopyTo(IID_PPV_ARGS(&g_pSwapChain));
            }
        ...
        /*pBackBuffer.ReleaseAndGetAddressOf();*/
        if (FAILED(hr))
            return E_FAIL;
